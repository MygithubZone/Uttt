...
public class AppInfoProviderUtils {
	...
	public AppInfoProviderUtils(Context c) {
		this.context = c;
	 ...
	 * 
	 * 根据SharedPreference的标志位，如果是第一次查询，那么直接在系统中遍历 如果不是第一次，那么直接从数据库进行查询即可
	 * 
	...
	public Map<String, AppUseStatics> getAppALL() {
		List<PackageInfo> pkgInfo = pm
				.getInstalledPackages(PackageManager.GET_ACTIVITIES);
		Map<String, AppUseStatics> result = new HashMap<String, AppUseStatics>();
		for (PackageInfo info : pkgInfo) {
	 ...
	 */
	public AppUseStatics getAppFromPkgName(PackageInfo info, PackageManager pm) {
